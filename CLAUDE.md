...
### Concrete Instances
- **Instance/Sets.v**: Category of setoids (not strict sets!). Monos and epis are characterized: `injectivity_is_monic` is a plain biconditional, but epis split across two universes — `surjective_implies_epic` holds at the universe of the objects, while the converse provably cannot be stated there (the truth-value object the classical probe needs has carrier `Type`, and Set's classifier lives one level up), so `sets_epic_implies_surjective`, the packaged `sets_epic_iff_surjective` and the balancedness corollary `sets_bimorphic_is_iso` quantify the `Epic` hypothesis against `SetoidMorphism_Lift` at the lifted placement (Instance/Sets/Classifier.v). All constructive — `∃` is `sigT`, so the preimages are data and no choice principle is used. `Instance/FinSet/Classifier.v`'s `finset_epic_iff_surjective` is the obstruction-free analogue, `Ω := 2` being an honest object of the category itself, with computable acceptance tests including the negative `finset_const_true_not_epic`
