...
The concrete algebraic categories are the exception that has to be written out by hand. Their object records carry `PropEquiv` (`Lib/Setoid/Propositional.v`), the property that the carrier's `≈` is logically a `Prop`-valued relation, because a `Prop`-valued relation on a carrier is carrier-sized where a `Type`-valued one is one universe up — the arithmetic the adjoint functor theorems turn on. The sort of `Prop` then contributes `Set+1` to each record, which would give the category built on it a third, redundant universe, so `CMon`, `Ab`, `Rig`, `Ring`, `Rg` and `Grp` are pinned BY HAND to `Category@{u p p}` with every field's instance written out, and the records' three universes read `(auxiliary, carrier, proof)` rather than `(carrier, proof, auxiliary)`. Annotations that give the three slots distinct levels must be permuted to match; all-equal ones such as `@{Set Set Set}` are unaffected. The authoritative statement of why any of this is needed is the size note in the header of `Structure/Complete.v`, which is where a disagreement between documents is to be resolved.
...
- Others: `Instance/Rel.v` (+ `Instance/Rel/Dagger.v`), `Instance/Props.v` (propositions under implication), `Instance/Theory/Lindenbaum.v`, `Instance/Lambda.v` (+ `Instance/Lambda/*`), `Instance/AST.v` (the free bicartesian closed category as a term model), `Instance/Fact.v` (the factorization category of a morphism), `Instance/Shapes.v` (shape-indexed tries; its `Shape` is unrelated to `Theory/Shapes.v`), `Instance/Roster.v` (the roster of standard large categories with their forgetful functors), `Instance/Concrete.v`, `Instance/Comp.v`, `Instance/Variety.v` (+ `Instance/Variety/*`: the derived-operator clone, the comparison with `Grp`, and the free-algebra adjunction), `Instance/ZX.v` (the only file with `Axiom`/`Parameter` declarations). Of these, only `Instance/Theory/Lindenbaum.v`, `Instance/Roster.v` and `Instance/Variety.v` head a bullet of their own in the index (`Instance/Lambda/*` heads one as a directory). `Instance/Variety.v` and its satellites are the one development whose principal constants are deliberately SPLIT across the `make print-assumptions` gate — the variety, the clone in setoid form and a depth-1 example are gated and closed, while five named constants and all of `Instance/Variety/GroupComparison.v` carry `functional_extensionality_dep` inherited from `Instance/Comp.v` and are recorded in docs/AXIOMS.md instead. `Instance/Variety/Free.v` extends that split and sharpens what it means: its GENERAL free-algebra adjunction is closed and gated, and only its four `GroupEq` instantiations carry the axiom — extensionality is the cost of BUILDING an `EqSignature`, not of using one.
