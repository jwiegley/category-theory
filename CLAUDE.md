...
- Others: `Instance/Rel.v` (+ `Instance/Rel/Dagger.v`), `Instance/Props.v` (propositions under implication), `Instance/Theory/Lindenbaum.v`, `Instance/Lambda.v` (+ `Instance/Lambda/*`), `Instance/AST.v` (the free bicartesian closed category as a term model), `Instance/Fact.v` (the factorization category of a morphism), `Instance/Shapes.v` (shape-indexed tries; its `Shape` is unrelated to `Theory/Shapes.v`), `Instance/Roster.v` (the roster of standard large categories with their forgetful functors), `Instance/Concrete.v`, `Instance/Comp.v`, `Instance/Variety.v` (+ `Instance/Variety/*`: the derived-operator clone and the comparison with `Grp`), `Instance/ZX.v` (the only file with `Axiom`/`Parameter` declarations). Of these, only `Instance/Theory/Lindenbaum.v`, `Instance/Roster.v` and `Instance/Variety.v` head a bullet of their own in the index (`Instance/Lambda/*` heads one as a directory). `Instance/Variety.v` and its satellites are the one development whose principal constants are deliberately SPLIT across the `make print-assumptions` gate — the variety, the clone in setoid form and a depth-1 example are gated and closed, while five named constants and all of `Instance/Variety/GroupComparison.v` carry `functional_extensionality_dep` inherited from `Instance/Comp.v` and are recorded in docs/AXIOMS.md instead.
