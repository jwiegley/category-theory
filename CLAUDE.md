...
- Topology and metric spaces: `Instance/Top.v` (+ `Instance/Top/*`, among them the finite case of Stone–Čech with the refutations of its adjoint-functor-theorem routes, `Instance/Top/StoneCech.v` and `Instance/Top/StoneCech/Refutations.v`, and the Prop-valued spaces `PTopCat` of `Instance/Top/Prop.v` with the subspace and quotient topologies over them as sliced adjoint inverses, `Instance/Top/Subspace.v`, their formable readings over the Type-valued `Top` being `Instance/Top/Subspace/TypeValued.v`), `Instance/Met.v` (+ `Instance/Met/*`). No development file outside `Instance/Top/*` and `Instance/Met*` requires the stdlib reals directly — nine files do, named in the `Instance/Top/FundamentalGroupoid.v` bullet of docs/INDEX.md, and `Instance/Top.v` itself is not among them; `Instance/Roster.v` reaches them through `Instance/Top/Homotopy.v`, and docs/AXIOMS.md carries the per-constant axiom footprint.
