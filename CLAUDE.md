...
### Concrete Instances
- **Instance/Ordinal.v**: The finite ordinals as categories — the uniform family `Ordinal n` (thin on `Fin.t n` with a Type-valued `≤`, following `Instance/Omega.v`'s `le_t` discipline), where the tree previously stopped at `_2`. Agreement isos `Ordinal 0 ≅ _0`, `Ordinal 1 ≅ _1`, `Ordinal 2 ≅ _2` in `Cat`, with `_3` and `_4` exported for downstream shapes; full and faithful embeddings `Ord_Incl : Ordinal n ⟶ Ordinal (S n)` and `Ord_Omega : Ordinal n ⟶ Omega` ("an ordinal is the set of its predecessors", categorically), commuting in the evident triangle. The Seven Sketches §3.2.1 morphism count is a closed formula by induction, not per-n enumeration: `ord_pairs n` enumerates the arrows duplicate-free and `ord_morphism_count : 2 * length (ord_pairs n) = n * (n+1)`. Riehl §1.6's freeness is stated as a direct UMP (no free-category machinery exists in tree): a path of n composable arrows (`OrdSteps`) generates `Functor_of_Steps : Ordinal (S n) ⟶ C` acting as the path on generating steps, every functor out of `Ordinal (S n)` is `Functor_Setoid`-equivalent to one so obtained (`Functor_of_Steps_of_Functor`), and naturality on the n generating steps suffices for naturality everywhere (`ord_functor_equiv_from_steps`); shape-4 diagrams specialize this at n=3, the four triangles commuting by thinness. On-the-nose functor↔path bijection is deliberately NOT claimed — a path's object part is a function on all of `nat`, so paths agreeing below n induce the same functor
