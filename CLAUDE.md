...
- **Instance/FinSet/Skeleton.v**: The skeleton equivalence of Mac Lane CWM §I.4 — `Set_f`, the full subcategory of `Sets` on the finite setoids, whose objects CARRY their counting (a chosen `θ_X : X ≅ Fin.t n` in `Sets`, data not an assertion, so the cardinal functor needs no choice principle); the inclusion `FinSet_Incl` shown full+faithful+essentially surjective and the equivalence obtained through `FF_ESO_Equivalence` rather than by hand, with `Card` (Mac Lane's `#`, `# f = θ_Y ∘ f ∘ θ_X⁻¹`) agreeing with the produced quasi-inverse up to conversion. Awodey §7.8's asymmetry is proved in both directions: `Card ◯ FinSet_Incl` is the identity by `eq_refl` on both maps with identity comparison cells, while `FinSet_Incl ◯ Card` is only naturally isomorphic to it — witnessed at `two_swap`, the canonical `Fin.t 2` carrying the transposition as its chosen bijection. Skeletality (`FinSet_skeletal : m ≅ n → m = n`, by an enumeration/counting argument, the tree's first statement about isomorphisms of `FinSet`) and Fong–Spivak §3.2.5 setf_cardinality as a complete isomorphism invariant (`setf_cardinality_iso_invariant`, `cardinality_complete`); `parity_two` — `nat` under equality of parity — is an object of setf_cardinality 2 with an infinite carrier
