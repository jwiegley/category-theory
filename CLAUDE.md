...
- **Instance/Top.v**: The category of topological spaces — spaces as setoids of points with a predicate-lattice topology (`IsOpen` over ≈-respecting predicates, arbitrary unions as dependent sums, the load-bearing `open_const`), continuous maps, `Top : Category`; discrete/indiscrete constructors with the constructive disclosure that indiscrete opens are the UNIFORM predicates (the `{∅, X}` presentation is not union-closed constructively); one-point terminal and empty initial (no zero object); monic ⟺ injective by point probes and the unconditional epi ⟺ surjective by an explicit cokernel-pair space (closed-form relation, no inductive closure, no decidability, no choice, in-universe), with the textbook two-point indiscrete probe recorded separately as decidability-scoped per the constructive-status question; Seven Sketches covers (`Covers`, `covers_nothing`); Hausdorff and compact-Hausdorff full subcategories, non-vacuous by the one-point space. Closed sets in Instance/Top/Closed.v: a closed set carries its complementing open as DATA with the setoid comparing the open (the constructive rendering of Riehl 1.3.14(iv)/1.4.4(v), disclosed in the header), the thin-category duality `Opens X ≅[Cat] (Closeds X)^op`, and the complementation natural isomorphism `OpensF ≅ ClosedsF` between the open- and closed-set functors `Top^op ⟶ Sets`, naturality being that preimages commute with complements
