...
Program Fixpoint expr_backward
  ...
  | Bottom => No
  | Equiv x y f g =>
    match term_denote objs arrs x y f, term_denote objs arrs x y g with
    | Some f', Some g' =>
      match ArrowList_beq (normalize f) (normalize g) with
      | true  => Yes
      | false => No
      end
    | _, _ => No
...
Next Obligation.
  destruct f; simpl; equalities.
  - clear expr_backward.
    induction g; simpl; equalities.
    + reflexivity.
    + destruct (arrs _); [|discriminate].
      destruct s, s; equalities.
    + simpl in Heq_anonymous.
      destruct (term_denote _ _ _ _ g1).
      destruct (term_denote _ _ _ _ g2).
      admit.
      destruct (arrs _); [|discriminate].
      destruct s, s; equalities.
