...
Program Definition ArrowList_Category : Category := {|
  obj := obj_idx;
  hom := fun x y =>
    ∃ l : ArrowList, ArrowList_dom l = x ∧ ArrowList_cod l = y;
...
Theorem normalize_well_typed {p dom cod f} :
...
Proof.
  ...
  destruct X0.
  destruct (normalize_sound (Term_well_typed_bool_sound _ _ _ H0) e2), p.
  rewrite e1, e4.
  red.
  rewrite <- e0, <- e3.
  rewrite H1 in e.
