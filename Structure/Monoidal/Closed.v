...
Definition flip `{@SymmetricMonoidal _ _} {x y z : C} `(f : x ~> y ⇒ z) :
  y ~> x ⇒ z := curry (uncurry f ∘ twist).
...
(*
Corollary eval_first {x y z : C} (f : x ~> y ⇒ z) :
...
Corollary uncurry_comp_r {x y z w : C} (f : z ~> w) (g : x ~> y ⇒ z) :
...
Corollary uncurry_comp {x y z w : C} (f : y ~> z ⇒ w) (g : x ~> y) :
