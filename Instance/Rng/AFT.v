...
(* nLab: https://ncatlab.org/nlab/show/adjoint+functor+theorem
   ...
   ** READ THIS BEFORE READING ANYTHING ELSE: ONE RESULT HERE IS NEW, TWO
      ARE CIRCULAR, AND THEY ARE KEPT APART ON PURPOSE
   ...
   Freyd's third hypothesis is a solution set, and the two fed to it below
   -- [Rng_Forget_Ab_solution_set_from_adjunction] and
   [Rng_Forget_solution_set_from_adjunction] -- are
   [solution_set_of_adjunction] (Adjunction/GAFT.v:366) applied to
   [free_rng_ab_adjunction] (Instance/Rng/Free.v:676) and to the retyping
   of [free_ring_via_ab_adjunction] (:874): the singleton family at the
   unit of THE VERY ADJUNCTION EACH APPLICATION IS MEANT TO PRODUCE.  So
   [free_rng_ab_via_GAFT] and [free_ring_via_GAFT] are genuine applications
   of a genuine theorem and are worthless as existence proofs -- strip
   Instance/Rng/Free.v and nothing in those two lines survives.  That is
   the same disclosure Instance/Grp/FreeAFT.v makes about
   [free_group_via_GAFT] and Adjunction/GAFT/Sets.v makes about its own
   instance; this file is the third to make it and does not weaken it.
   ...
   ** THE TWO HIDDEN CHECKBOXES IN THE ISSUE BODY
   ...
   application in the tree."  ADDRESSED, at two functors, and with the
   circularity disclosed: the solution sets below are not Mac Lane's, and
   the derivation is therefore an application of the theorem and not an
   independent construction.  Awodey's exercise is satisfied in its literal
   reading -- the theorem has a genuine application -- and NOT in the
   reading a reader might want, where the free ring is OBTAINED from the
   theorem.  The same sentence is true of Instance/Grp/FreeAFT.v, which got
   there first at [Grp].
   ...
   ** WHAT IS DELIVERED
   ...
   5. Two solution sets, NAMED so their circularity is visible at every use
      site, and the two GAFT applications [free_rng_ab_via_GAFT] and
      [free_ring_via_GAFT].
   6. Three comparison clauses at [≈], each with its named comparison
      morphism and its unit equation: against [FreeRngAb], against
      [free_ring_via_ab] and against [free_ring_via_mon].
   ...
   ** NOT DELIVERED
   ...
   (3) NO NON-CIRCULAR SOLUTION SET.  Mac Lane's own -- the subrings
   generated by the image of a set, cut down by a cardinality bound -- needs
   a generated-subring API that does not exist over [RingObject]; the
   analogous gap at [Grp] is filed as #1309 together with the
   universe-minimization artifact of Instance/Discrete.v:59's
   [DiscreteCat_Functor], and the [Rng] case would meet the same two
   obstacles.  Nothing here attempts it.  (4) THE GRAPH HALF OF THE ISSUE
   IS NOT TOUCHED.  Producing the left adjoint to the categories-to-graphs
...
(* CIRCULAR, as the header says.  Each is named so that the circularity is
   visible at every use site rather than buried in a term: the family is
   the singleton at the unit of the adjunction the theorem is being asked
   to produce. *)
...
(* Second half: the free ring on a set. *)
...
Definition free_ring_via_GAFT_agrees_mon :
  `1 free_ring_via_GAFT ≈ free_ring_via_mon :=
  left_adjoints_agree (`2 free_ring_via_GAFT) free_ring_via_mon_adjunction_set.
...
Example free_ring_via_GAFT_agrees_mon_via_left_adjoint_iso :
  `1 free_ring_via_GAFT ≈ free_ring_via_mon :=
  left_adjoint_iso Rng_Forget _ free_ring_via_mon (`2 free_ring_via_GAFT)
...
Definition free_ring_via_GAFT_comparison_mon (X : Sets)
  : `1 free_ring_via_GAFT X ~{Rng}~> free_ring_via_mon X :=
  adj_left_compare (`2 free_ring_via_GAFT) free_ring_via_mon_adjunction_set X.
