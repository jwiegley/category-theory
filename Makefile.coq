...
# coq_makefile -f _CoqProject . Category.v Coq.v CpdtTactics.v Functors.v Lambda.v Monoidal.v Notes.v Products.v Tactics.v Utils.v 
#
...
uninstall_me.sh:
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Hask && rm -f $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Hask" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
