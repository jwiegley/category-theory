...
| `dual_functor_no_right_adjoint`, `dual_not_left_adjoint_of_op` (`Instance/FdVect/NoRightAdjoint.v`) | `CoordSpanProper F` — a functional on `F^ℕ` outside the span of the coordinate projections | NO constructive witness is POSSIBLE, not merely absent: the premise is classically true for every field (Erdős–Kaplansky) but, for F = ℚ, independent of ZF + DC — in a model where every set of reals has the Baire property, every additive map `ℚ^ℕ → ℚ` is continuous (Pettis) and so a finite combination of coordinates — so unlike the rows above it will never get an in-tree inhabitant without a choice principle (analysis in the file's header, not formalized); the positive half (`dual_vct_AdjointOnTheRight`, `dual_vct_Continuous`) is unconditional |
