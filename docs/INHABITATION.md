...
|--------|---------------------|-----------------------|
| `GAFT` (solution-set form) | `Complete C` | witnessed at `Sets` — `Sets_Complete` (`Instance/Sets/Complete.v`) and, since #329, `Sets_Cocomplete` (`Instance/Sets/Cocomplete.v`), the first hypothesis-free inhabitant of either class; the theorem is stated for an arbitrary `C`. An earlier revision of this cell added "and no OTHER category has one"; measured against the tree on 2026-09-12 that is false on four counts, all unconditional (no `Complete` premise of their own): `Grp_Complete` (`Instance/Grp/Limit.v:691`), `Presheaf_Complete` and `Fun_Sets_Complete` (`Instance/Fun/Limit.v:451`, `:458`) and `Subsets_Complete` (`Instance/Powerset.v:637`) — criterion: `grep -E '(Definition|Instance) [A-Za-z_0-9]*Complete '` over the tree outside `Test/`, keeping those whose type is `@Complete <category>` with no `@Complete` argument, which excludes the transported `Arrow_Complete`, `Coslice_Complete`, `Res_Complete`, `Product_Complete` and `reflective_Complete`. Since #442 GAFT is also APPLIED at `Grp` (`free_group_via_GAFT`, `Instance/Grp/FreeAFT.v`), on `Grp_Complete` and `Grp_Forget_continuous`; that application's SOLUTION SET is derived from the free-group adjunction it is meant to produce, so it is a genuine instantiation and not an independent existence proof, and the file says so in capitals. This cell read "no `Complete`/`Cocomplete` instance exists in-tree" until #329: the `Cocomplete` half was falsified by that issue, the `Complete` half had already been stale since `Sets_Complete` landed |
...
| `SAFT` | `SolutionSet` + `Cogenerator` + `SubobjectIndex` | none of the three is inhabited. Since #437 `SAFT` IS applied, exactly once and conditionally, by `saft_representable` (`Adjunction/Representability/Sets.v`), which inherits all three premises and so has no witness either; this cell read "`SAFT` is never applied" until then, and the same sentence in `Adjunction/GAFT/Sets.v` and `Instance/Sets/Complete.v` is corrected in place |
