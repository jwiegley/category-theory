...
| `left_adjoint_of_representable_Sets` (`Adjunction/Representability/Sets.v`, #437) | `HasSetsCopowers C` — a copower of each object by each SETOID, i.e. by an object of `Sets` | NOT inhabited, and the obstruction is structural rather than missing work: every copower in tree (`Structure/Limit/Power.v`, #366) is indexed by a bare `Type` and so yields plain functions out of the index, where a `Sets`-valued adjunction needs morphisms respecting `≈` (`Test/ProbeRepresentability437.v`'s N5 pins the type mismatch). By #348's `adjunction_iff_pointwise_representable` the hypothesis IS "the represented functor has a left adjoint", so the converse of §V.8 Ex 1 is conditional by construction. The rest of #437 is unconditional: `representability_theorem` and `representability_iff` are witnessed at `Sets` twice over (`Sets_points_repr`, `Sets_Id_repr`, and `Sets_Id_repr_of_adjoint` by the forward direction of Ex 1) |
